 ...
 *
 * Modified by Zeng Zichao (2024-2026) to add clade-collapse rendering
 * support for FigTreeKit.  Changes: added constructCollapsedNode() and
 * the collapse-check branch in constructNode(); all other code is
 * unchanged from FigTree v1.4.4.
 *
...
//            constructNodeAreas(tree, root, new Area(), cache);
    ...
    private Point2D constructNode(RootedTree tree, Node node,
        ...
        if (!tree.isExternal(node)) {
            ...
            // Collapse support: if node has !collapse annotation, draw a triangle
            // pointing outward (toward the tips) instead of recursing into children.
            if (collapsedAttributeName != null && node.getAttribute(collapsedAttributeName) != null) {
                nodePoint = constructCollapsedNode(tree, node, angleStart, angleFinish,
                        xPosition, yPosition, length, cache);
                Point2D nodeShapePoint = new Point2D.Double(
                        xPosition + ((length - 1.0) * directionX),
...
//			            float p = (height - childHeight) / (nodeHeight - childHeight);
    ...
    /**
     * Construct a collapsed node (triangle) for radial layout.
     *
     * The triangle apex is at the node's position (xPosition + length along the
     * branch angle). The base is at radius {@code maxRadius = nodeRadius +
     * (height - tipHeight)}, spanning from angleStart to angleFinish.
     * In jebl's time-backward coordinates, {@code height = getHeight(node)}
     * and {@code tipHeight = getMinTipHeight(tree, node)}, so
     * {@code height - tipHeight} equals the subtree depth (max distance from
     * node to any tip in its subtree), which is always >= 0, making the
     * triangle point outward (toward the tips).
     */
    private Point2D constructCollapsedNode(RootedTree tree, Node node,
                                           double angleStart, double angleFinish,
                                           double xPosition, double yPosition,
                                           double length, TreeLayoutCache cache)
            throws Graph.NoEdgeException {

        Object[] values = (Object[]) node.getAttribute(collapsedAttributeName);
        // values[0] = tipName (String), values[1] = tipHeight (Double)
        ...
        // nodePoint: the apex of the triangle (current node position)
        Point2D nodePoint = new Point2D.Double(
                xPosition + (length * directionX),
                yPosition + (length * directionY));

        // nodeRadius: actual radial distance of the node from the origin
        double nodeRadius = Math.sqrt(nodePoint.getX() * nodePoint.getX()
                + nodePoint.getY() * nodePoint.getY());
        // maxRadius: radial distance of the triangle base (= farthest tip radius)
        // height - tipHeight = subtree depth (always >= 0)
        double maxRadius = nodeRadius + (height - tipHeight);

        // Triangle vertices:
        //   apex  = nodePoint
        //   base1 = (maxRadius * cos(angleStart), maxRadius * sin(angleStart))
        //   base2 = (maxRadius * cos(angleFinish), maxRadius * sin(angleFinish))
        // The base is connected by an arc centered at the origin.
        GeneralPath collapsedShape = new GeneralPath();

        float x0 = (float) nodePoint.getX();
        float y0 = (float) nodePoint.getY();
        float x1 = (float) (maxRadius * Math.cos(angleStart));
        float y1 = (float) (maxRadius * Math.sin(angleStart));

        collapsedShape.moveTo(x0, y0);
        collapsedShape.lineTo(x1, y1);

        // Arc along the base, from angleStart to angleFinish.
        // Arc2D uses degrees, with extent measured counterclockwise.
        if (Math.abs(angleFinish - angleStart) > 1.0E-9) {
            double startDeg = Math.toDegrees(angleStart);
            double extentDeg = Math.toDegrees(angleFinish - angleStart);
            Arc2D arc = new Arc2D.Double();
            arc.setArcByCenter(0.0, 0.0, maxRadius, startDeg, extentDeg, Arc2D.OPEN);
            collapsedShape.append(arc, true);
        }

        float x2 = (float) (maxRadius * Math.cos(angleFinish));
        float y2 = (float) (maxRadius * Math.sin(angleFinish));
        collapsedShape.lineTo(x2, y2);
        collapsedShape.closePath();

        cache.collapsedShapes.put(node, collapsedShape);

        // Tip label path: place the label just beyond the triangle base,
        // along the branch angle (so the collapsed-clade name shows up).
        Point2D tipLabelPoint = new Point2D.Double(
                maxRadius * directionX, maxRadius * directionY);
        Point2D tipLabelEnd = new Point2D.Double(
                (maxRadius + 1.0) * directionX, (maxRadius + 1.0) * directionY);
        Line2D tipLabelPath = new Line2D.Double(tipLabelPoint, tipLabelEnd);
        cache.tipLabelPaths.put(node, tipLabelPath);

        // Node label path: just beyond the apex, along the branch angle.
        Point2D nodeLabelEnd = new Point2D.Double(
                (nodeRadius + 1.0) * directionX, (nodeRadius + 1.0) * directionY);
        Line2D nodeLabelPath = new Line2D.Double(nodePoint, nodeLabelEnd);
        cache.nodeLabelPaths.put(node, nodeLabelPath);
